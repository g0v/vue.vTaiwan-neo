  ...
  <div class="transcript-panel flex h-full flex-col border-l border-gray-200 bg-white">
    ...
    <div class="flex-shrink-0 border-b border-gray-200 bg-gray-50 p-4">
      <div class="mb-2 flex items-center justify-between">
        ...
        <button @click="$emit('close')" class="rounded-lg p-2 transition hover:bg-gray-200 md:hidden" :title="$t('transcript.hideTranscript')">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button @click="toggleAutoScroll" :class="['rounded-full px-3 py-1 text-xs transition', autoScroll ? 'bg-jade-green text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300']">
    ...
    <div ref="transcriptContent" class="max-h-[50vh] flex-shrink-0 space-y-2 overflow-y-auto p-4" :class="fontSizeClass">
      ...
      <div v-for="(entry, index) in Object.values(transcriptData)" :key="entry.timestamp" class="transcript-entry rounded-lg border border-gray-200 p-3 transition hover:border-gray-300">
    ...
    <div class="flex-shrink-0 border-t border-gray-200 bg-gray-50 p-3">
      <div class="flex items-center justify-between text-xs text-gray-500">
        ...
        <div class="flex items-center gap-2">
          <div :class="['h-2 w-2 rounded-full', isConnected ? 'bg-jade-green' : 'bg-red-500']"></div>
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="mx-4 flex max-h-[90vh] w-full max-w-4xl flex-col rounded-lg bg-white shadow-xl">
        ...
        <div class="flex items-center justify-between border-b border-gray-200 p-6">
          ...
          <button @click="cancelEdit" class="rounded-lg p-2 transition hover:bg-gray-100" :title="$t('transcript.close')">
        ...
        <div class="flex items-center justify-end gap-3 border-t border-gray-200 p-6">
          <button @click="cancelEdit" class="rounded-lg bg-gray-300 px-6 py-3 text-base text-gray-700 transition hover:bg-gray-400">
            {{ $t('transcript.cancel') }}
          </button>
          <button @click="saveEntry" class="rounded-lg bg-jade-green px-6 py-3 text-base text-white transition hover:bg-jade-green/90">
            {{ $t('transcript.save') }}
          </button>
        </div>
      </div>
